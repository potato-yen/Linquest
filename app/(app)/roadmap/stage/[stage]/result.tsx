...
import { getRoadmapBank } from '../../../../../lib/roadmap/service';
...
export default function StageResult() {
  ...
  const lastStage = getRoadmapBank().config.levels.reduce((max, level) => Math.max(max, level.stage_end), 1);
  const nextProgress = deriveRoadmapProgressState(stageNum + 1, lastStage);
  const completedAllStages = unlocked && nextProgress.isComplete;
  ...
  return (
    <ScreenScaffold scroll>
      <Text variant="h1">
        {completedAllStages ? '已完成全部關卡' : unlocked ? '解鎖下一關' : '本關未解鎖'}
      </Text>
      <Card padding={5}>
      ...
      {completedAllStages ? (
        <Text color="muted">門檻 80%。你已通過最後一關。</Text>
      ) : unlocked ? (
      ...
      <View style={{ gap: space[2] }}>
        <Button
          ...
          onPress={() => router.replace(
            completedAllStages
              ? '/(app)/roadmap'
              : `/(app)/roadmap/stage/${unlocked ? stageNum + 1 : stageNum}`,
          )}
        />
        <Button
          title="返回山徑"
          variant="ghost"
          onPress={() => router.replace(
            unlocked && !completedAllStages
              ? `/(app)/roadmap?unlocked=${stageNum + 1}`
              : '/(app)/roadmap',
          )}
        />
      </View>
