alter table public.hex_tiles
  ...
  add column if not exists active_challenge_kind text;

alter table public.hex_tiles
  drop constraint if exists hex_tiles_active_challenge_kind_check;

alter table public.hex_tiles
  add constraint hex_tiles_active_challenge_kind_check
  check (
    active_challenge_kind is null
    or active_challenge_kind in (
      'capture_normal',
      'reverse_normal',
      'capture_multiplier',
      'reverse_multiplier',
...
drop function if exists public.attempt_capture(uuid, uuid, uuid, integer, integer, boolean);
...
drop function if exists public.resolve_challenge(uuid, uuid, uuid, uuid, text, boolean, integer, integer, integer, integer, timestamptz);
