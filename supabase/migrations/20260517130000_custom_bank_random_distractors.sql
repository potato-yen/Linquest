-- Custom-bank question model change (teacher console, frontend-driven).
--
-- Old model: teacher uploaded prompt, correct_answer, distractor_1..3,
-- difficulty. New model (user decision 2026-05-17): a question is just
-- 中文(prompt) + 英文(correct_answer) + 詞性(part_of_speech). The 3
-- distractors are NOT authored — they are sampled at answer time from the
-- other correct answers in the same bank (mirrors how roadmap already
-- works: lib/roadmap/sheet-source.ts sampleDistractors).
--
-- Scope: ONLY teacher custom banks. Roadmap (Google-Sheet bank) and the
-- official `questions`-table seed format are untouched.
--
-- public.questions.distractors is `text[] NOT NULL CHECK(len=3)` and is
-- shared by official banks / territory / battle. We do NOT relax that
-- table-wide constraint. Instead custom-bank rows store a 3-element
-- placeholder ARRAY['','','']; territory/battle ignore the stored value
-- for custom banks and sample real distractors client-side.
--
-- Only the RPC changes (immutable-migrations rule → new file, same
-- signature so the existing grant carries).
...
begin
  ...
  -- Each row: non-empty prompt + correct_answer. part_of_speech optional;
  -- if present it must be a string. No distractors / difficulty authored.
  ...
  -- Distractors are sampled from sibling correct answers at answer time,
  -- so the bank needs at least 4 distinct answers (1 correct + 3 picks).
  ...
  select
    ...
    elem->>'prompt',
    elem->>'correct_answer',
    array['', '', '']::text[],   -- placeholder; real distractors sampled client-side
    jsonb_strip_nulls(jsonb_build_object(
      'part_of_speech', nullif(elem->'meta'->>'part_of_speech', ''),
      'difficulty', 'standard'   -- keep filter compatibility; UI ignores it for custom
    ))
  from jsonb_array_elements(p_rows) elem;
